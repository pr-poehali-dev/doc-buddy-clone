...
export default function Index() {
  ...
  const generateDocContent = (docId: string): string => {
    ...
    const services = form.hasAnalytics ? "Яндекс.Метрика, Google Analytics" : ph("СЕРВИСЫ");
...
5.1. Обработка заявки, обратной связи или запроса пользователя: ${ph("ПДН, НАПРИМЕР: имя, телефон, email, комментарий")}.
5.2. Обратный звонок: ${ph("ПДН, НАПРИМЕР: имя, телефон")}.
5.3. Заключение и исполнение договора/заказа, если применимо: ${ph("ПДН, НАПРИМЕР: ФИО, телефон, email, адрес доставки, состав заказа")}.
5.4. Регистрация и использование личного кабинета, если применимо: ${ph("ПДН, НАПРИМЕР: имя, email, телефон, логин, история заказов")}.
5.5. Онлайн-оплата, если применимо: ${ph("ПДН, ПЕРЕДАВАЕМЫЕ ПЛАТЕЖНОМУ АГРЕГАТОРУ")}. Оператор не хранит полные данные банковских карт, если иное не предусмотрено фактическим процессом.
5.6. Доставка, если применимо: ${ph("ПДН, НАПРИМЕР: ФИО, телефон, адрес доставки")}.
5.7. Рассылка информационных или рекламных сообщений, если применимо: ${ph("ПДН, НАПРИМЕР: имя, email, телефон, мессенджер")}.
...
- заявки и обращения: ${ph("СРОК")};
- договорные и бухгалтерские документы: ${ph("СРОК С УЧЕТОМ ЗАКОНА")};
- данные личного кабинета: ${ph("СРОК")};
- данные рассылки: до отзыва согласия или ${ph("СРОК")};
- cookie/аналитические данные: ${ph("СРОКИ ПО СЕРВИСАМ")};
- резюме кандидатов: ${ph("СРОК")}.
...
Категории третьих лиц: ${ph("ХОСТИНГ-ПРОВАЙДЕР, CRM, СЕРВИС РАССЫЛКИ, ПЛАТЕЖНЫЙ АГРЕГАТОР, КУРЬЕРСКАЯ СЛУЖБА, ОНЛАЙН-ЧАТ, АНАЛИТИКА, ПОДРЯДЧИКИ")}.
Передаваемые данные и цели передачи: ${ph("ПО КАЖДОМУ ТРЕТЬЕМУ ЛИЦУ")}.

12. Трансграничная передача
Трансграничная передача персональных данных осуществляется только при наличии законного основания и после выполнения необходимых требований законодательства. На дату публикации Политики: ${ph("ЕСТЬ ЛИ ЗАРУБЕЖНЫЕ СЕРВИСЫ И СТРАНЫ")}.

13. Меры защиты персональных данных
Оператор принимает необходимые правовые, организационные и технические меры для защиты персональных данных от неправомерного доступа, уничтожения, изменения, блокирования, копирования, предоставления, распространения и иных неправомерных действий. Конкретные меры включают: ${ph("МЕРЫ, НАПРИМЕР: разграничение доступа, пароли, HTTPS, резервное копирование, договоры с обработчиками, журналирование, антивирусная защита")}.
...
На Сайте могут использоваться следующие сервисы: ${ph("ЯНДЕКС.МЕТРИКА / GOOGLE ANALYTICS / ПИКСЕЛИ / CALL-TRACKING / ОНЛАЙН-ЧАТ / CRM / ИНЫЕ СКРИПТЫ")}. До получения согласия пользователя не должны загружаться необязательные аналитические и маркетинговые скрипты, если это применимо.
...
8. Отзыв согласия
Пользователь может изменить выбор cookie через ссылку/кнопку ${ph("МЕХАНИЗМ: «Настройки cookie»")} на сайте или направить обращение на ${email}.

9. Связь с Политикой ПДн
Обработка данных, полученных через cookie и аналогичные технологии, осуществляется в соответствии с Политикой обработки персональных данных, размещенной по адресу: ${ph("ССЫЛКУ")}.`;
    }
...
Цели обработки: обработка заявки/обращения, обратная связь, предоставление ответа, подготовка предложения, заключение и исполнение договора/заказа, а также иные конкретные цели, указанные рядом с соответствующей формой: ${ph("ЦЕЛИ ПО ФОРМАМ")}.
Перечень персональных данных: ${ph("ДАННЫЕ ПО ФОРМАМ: имя, телефон, email, комментарий, город, адрес, файл, IP, user agent и т.д.")}.
...
Третьи лица/категории третьих лиц, которым могут передаваться данные: ${ph("CRM, ХОСТИНГ, СЕРВИС РАССЫЛКИ, ПЛАТЕЖНЫЙ АГРЕГАТОР, КУРЬЕРСКУЮ СЛУЖБУ, ПОДРЯДЧИКОВ ИЛИ УКАЗАТЬ «НЕ ПЕРЕДАЮТСЯ», ЕСЛИ ЭТО ТАК")}.
Срок действия согласия: до достижения целей обработки, до истечения срока хранения ${ph("СРОК")} или до отзыва согласия, если отсутствуют иные законные основания для обработки.
...
Сервисы и третьи лица: ${ph("ЯНДЕКС.МЕТРИКА, GOOGLE ANALYTICS, РЕКЛАМНЫЕ ПИКСЕЛИ, CALL-TRACKING, ОНЛАЙН-ЧАТ, ИНЫЕ СЕРВИСЫ")}.
Срок действия согласия: до изменения пользователем настроек cookie, до отзыва согласия или ${ph("СРОК")}.
Согласие может быть отозвано через настройки cookie на сайте ${ph("МЕХАНИЗМ")} или путем обращения на ${email}.`;
    }
...
Каналы рассылки: ${ph("EMAIL, SMS, ТЕЛЕФОННЫЕ ЗВОНКИ, МЕССЕНДЖЕРЫ, PUSH, ИНЫЕ КАНАЛЫ")}.
Виды сообщений: новости, акции, специальные предложения, промокоды, информация о товарах/услугах, приглашения на мероприятия и иные рекламные сообщения: ${ph("УКАЗАТЬ")}.
Обрабатываемые данные: ${ph("имя, email, телефон, аккаунт в мессенджере и иные данные, необходимые для рассылки")}.
Срок действия согласия: до отзыва согласия или ${ph("СРОК")}.
...
Цель публикации: размещение отзыва, кейса, фото/видео, информации о специалисте, клиентском опыте или иной материал: ${ph("КОНКРЕТНУЮ ЦЕЛЬ")}.
Перечень персональных данных, разрешенных для распространения: ${ph("ФИО, имя, должность, компания, фото, видео, текст отзыва, ссылка на профиль и т.д.")}.
Условия и запреты распространения: ${ph("УСЛОВИЯ/ЗАПРЕТЫ ИЛИ «НЕ УСТАНОВЛЕНЫ»")}.
Срок действия согласия: ${ph("СРОК")} или до отзыва согласия.
...
1. Предмет
Настоящий документ регулирует порядок использования сайта ${site} и, если применимо, порядок заказа товаров/услуг, регистрации личного кабинета и оплаты. Конкретное назначение документа: ${ph("ПОЛЬЗОВАТЕЛЬСКОЕ СОГЛАШЕНИЕ / ПУБЛИЧНАЯ ОФЕРТА")}.

2. Сведения о владельце сайта
${company}, ИНН ${inn}, ОГРН/ОГРНИП ${ogrn}, адрес: ${address}, email: ${email}.

3. Регистрация и личный кабинет
Раздел применяется, если на сайте есть личный кабинет. Пользователь обязуется предоставлять достоверные данные и сохранять конфиденциальность учетных данных. ${ph("ПОРЯДОК РЕГИСТРАЦИИ И УДАЛЕНИЯ АККАУНТА")}.

4. Заказ, оплата и исполнение
Раздел применяется, если сайт принимает заказы или оплату. ${ph("ТОВАРЫ/УСЛУГИ, ЦЕНЫ, ПОРЯДОК ОПЛАТЫ, СРОКИ, ДОСТАВКУ, ВОЗВРАТЫ")}.

5. Ограничения ответственности
${ph("РЕАЛЬНЫЕ ОГРАНИЧЕНИЯ, НЕ НАРУШАЮЩИЕ ПРАВА ПОТРЕБИТЕЛЕЙ")}.
          ...
          {step === 3 && generated && (
            <div className="animate-fade-in">
              ...
              <div className="mt-6 flex justify-between items-center">
                <button
                  onClick={() => { setStep(1); setGenerated(false); setForm({ companyName: "", companyType: "юридическое лицо", website: "", email: "", phone: "", address: "", inn: "", ogrn: "", hasAnalytics: true, hasPayments: false, hasCookies: true, hasFeedbackForm: true, hasMailing: false, hasPublishing: false, hasAccount: false }); }}
