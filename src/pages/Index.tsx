            ...
            <div className="card-glass rounded-2xl p-8 animate-fade-in">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2 pt-2">
                  ...
                  <p className="text-xs text-gray-400 mb-3">Необязательно. Если оставить поле пустым, в документе останется метка <span className="text-[#1a4fd6] font-medium">[УКАЗАТЬ ...]</span> для самостоятельного заполнения.</p>
                </div>
                <div className="md:col-span-2">
                  <label className="text-sm font-semibold text-gray-600 block mb-1.5">Состав данных по формам</label>
                  <input
                    type="text"
                    value={form.formData}
                    onChange={(e) => setForm({ ...form, formData: e.target.value })}
                    placeholder="Имя, телефон, email"
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-[#1a4fd6] transition-all bg-white"
                  />
                </div>
                <div className="md:col-span-2">
                  <label className="text-sm font-semibold text-gray-600 block mb-1.5">Сервисы аналитики</label>
                  <input
                    type="text"
                    value={form.analyticsServices}
                    onChange={(e) => setForm({ ...form, analyticsServices: e.target.value })}
                    placeholder="Яндекс.Метрика, cookie"
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-[#1a4fd6] transition-all bg-white"
                  />
                </div>
                <div className="md:col-span-2">
                  <label className="text-sm font-semibold text-gray-600 block mb-1.5">Третьи лица, которым передаются данные</label>
                  <input
                    type="text"
                    value={form.thirdParties}
                    onChange={(e) => setForm({ ...form, thirdParties: e.target.value })}
                    placeholder="Например: CRM, хостинг, сервис рассылки — или «не передаются»"
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-[#1a4fd6] transition-all bg-white"
                  />
                </div>
                <div className="md:col-span-2">
                  <label className="text-sm font-semibold text-gray-600 block mb-1.5">Срок хранения данных</label>
                  <input
                    type="text"
                    value={form.retentionPeriod}
                    onChange={(e) => setForm({ ...form, retentionPeriod: e.target.value })}
                    placeholder="Например: 3 года или до отзыва согласия"
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-[#1a4fd6] transition-all bg-white"
                  />
                </div>
              </div>
